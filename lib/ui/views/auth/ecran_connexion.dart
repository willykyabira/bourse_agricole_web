...
class _EcranConnexionState extends State<EcranConnexion> {
  ...
  Future<void> _verifierConnexion() async {
    // 1. Validation : Bloque l'envoi si champs vides
    ...
    try {
      ...
      if (res.user != null && mounted) {
        ...
        if (role == 'admin') {
          ...
          Navigator.pushReplacementNamed(context, '/mouvements'); 
        } else {
          Navigator.pushReplacementNamed(context, '/');
        }
      }
    } on AuthException catch (e) {
      // 2. Gestion erreurs d'identification
      _notifierErreur("Email ou mot de passe incorrect.");
    } on Exception catch (e) {
      // 3. Gestion erreurs réseau (ClientException, etc.)
      if (e.toString().contains("ClientException") || e.toString().contains("Failed to fetch")) {
         _notifierErreur("Problème de connexion. Vérifiez votre internet.");
      } else {
         _notifierErreur("Une erreur technique est survenue.");
      }
    } finally {
  ...
  Widget _buildServicesPanel() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("BAN, Bourse Agricole Numérique, une plateforme numérique facilitant la vente et l’achat de produits agricoles en Ituri", 
  ...
  // Ajout du paramètre validator ici
  Widget _buildInputField({
    ...
    return TextFormField(
      ...
      validator: validator, // Application de la règle de validation
